...
Version: 1.0
  Date: 2026-04-10
  Description: Markdown 渲染函数与工具（来自 office_middle_json_mkcontent.py）
'''
...
from .constants import _BlockType, _ContentType, _ContentTypeV2, _MakeMode


# ── LaTeX 分隔符配置（第 4993-5018 行）──
def _get_latex_delimiter_config():
    import json as _json
    config_file = os.path.join(os.path.expanduser('~'), os.getenv('MINERU_TOOLS_CONFIG_JSON', 'mineru.json'))
    if not os.path.exists(config_file):
        return None
    try:
        with open(config_file, 'r', encoding='utf-8') as _f:
            return _json.load(_f).get('latex-delimiter-config', None)
    except Exception:
        return None


latex_delimiters_config = _get_latex_delimiter_config()
...
def _needs_markdown_it_boundary_space(prev_part: dict, next_part: dict) -> bool:
    ...
    if next_part.get('span_type') in {
        _ContentType.HYPERLINK,
        _ContentType.INLINE_EQUATION,
        _ContentType.INTERLINE_EQUATION,
    }:
        return False
...
def _join_rendered_parts(parts: list[dict]) -> str:
    ...
    for i, part in enumerate(parts):
        ...
        if span_type == _ContentType.INLINE_EQUATION:
...
def merge_para_with_text(para_block):
    ...
    parts = []
    if para_block['type'] == _BlockType.TITLE:
    ...
    for line in para_block['lines']:
        for span in line['spans']:
            ...
            if span_type == _ContentType.TEXT:
                _append_text_part(parts, span['content'], span_style)
            elif span_type == _ContentType.INLINE_EQUATION:
            ...
            elif span_type == _ContentType.INTERLINE_EQUATION:
            ...
            elif span_type == _ContentType.HYPERLINK:
...
def _flatten_list_items(list_block):
    ...
    for block in list_block.get('blocks', []):
        if block['type'] in [_BlockType.LIST, _BlockType.INDEX]:
    ...
    for child in index_block.get('blocks', []):
        if child.get('type') == _BlockType.INDEX:
            ...
            for i, (content, span_type, _) in enumerate(span_items):
                if span_type != _ContentType.INLINE_EQUATION and '\t' in content:
                    last_tab_span_idx = i
            ...
            non_eq_styles = [
                ...
                if content and span_type != _ContentType.INLINE_EQUATION
            ]
            uniform_style = None
            if non_eq_styles:
...
def _mk_blocks_to_markdown(para_blocks, make_mode, img_buket_path='', page_idx=None):
    page_markdown = []
    for para_block in para_blocks:
        para_text = ''
        para_type = para_block['type']
        if para_type in [_BlockType.TEXT, _BlockType.INTERLINE_EQUATION]:
            para_text = merge_para_with_text(para_block)
            if para_type == _BlockType.TEXT:
        ...
        elif para_type == _BlockType.LIST:
            para_text = merge_list_to_markdown(para_block)
        elif para_type == _BlockType.INDEX:
            para_text = merge_index_to_markdown(para_block)
        elif para_type == _BlockType.TITLE:
        ...
        elif para_type == _BlockType.IMAGE:
            if make_mode == _MakeMode.NLP_MD:
                continue
            elif make_mode == _MakeMode.MM_MD:
                for block in para_block['blocks']:  # 1st.拼image_body
                    if block['type'] == _BlockType.IMAGE_BODY:
                        for line in block['lines']:
                            for span in line['spans']:
                                if span['type'] == _ContentType.IMAGE:
                                    img_b64 = span.get('image_base64', '')
                                    if img_b64:
                                        # base64 mode: embed as data-URI
                                        para_text += f"![image]({img_b64})"
                                    elif span.get('image_path', ''):
                                        para_text += f"![]({img_buket_path}/{span['image_path']})"
                for block in para_block['blocks']:  # 2nd.拼image_caption
                    if block['type'] == _BlockType.IMAGE_CAPTION:
                        para_text += '  \n' + merge_para_with_text(block)

        elif para_type == _BlockType.TABLE:
            if make_mode == _MakeMode.NLP_MD:
                continue
            elif make_mode == _MakeMode.MM_MD:
                for block in para_block['blocks']:  # 1st.拼table_body
                    if block['type'] == _BlockType.TABLE_BODY:
                        for line in block['lines']:
                            for span in line['spans']:
                                if span['type'] == _ContentType.TABLE:
                                    para_text += f"\n{_format_embedded_html(span['html'], img_buket_path)}\n"
                for block in para_block['blocks']:  # 2nd.拼table_caption
                    if block['type'] == _BlockType.TABLE_CAPTION:
                        para_text += '  \n' + merge_para_with_text(block)
        elif para_type == _BlockType.CHART:
            if make_mode == _MakeMode.NLP_MD:
                continue
            elif make_mode == _MakeMode.MM_MD:
                ...
                for block in para_block['blocks']:
                    if block['type'] == _BlockType.CHART_CAPTION:
                        para_text += '  \n' + merge_para_with_text(block)
        if para_text.strip() == '':
            continue
        else:
...
def make_blocks_to_content_list(para_block, img_buket_path, page_idx):
    ...
    elif para_type == _BlockType.INTERLINE_EQUATION:
        para_content = {
            'type': _ContentType.EQUATION,
            'text': merge_para_with_text(para_block),
            'text_format': 'latex',
        }
    elif para_type == _BlockType.IMAGE:
        para_content = {'type': _ContentType.IMAGE, 'img_path': '', _BlockType.IMAGE_CAPTION: []}
        for block in para_block['blocks']:
            if block['type'] == _BlockType.IMAGE_BODY:
                for line in block['lines']:
                    for span in line['spans']:
            ...
            if block['type'] == _BlockType.IMAGE_CAPTION:
                para_content[_BlockType.IMAGE_CAPTION].append(merge_para_with_text(block))
    elif para_type == _BlockType.TABLE:
        para_content = {'type': _ContentType.TABLE, _BlockType.TABLE_CAPTION: []}
        for block in para_block['blocks']:
            if block['type'] == _BlockType.TABLE_BODY:
                for line in block['lines']:
                    for span in line['spans']:
                        if span['type'] == _ContentType.TABLE:
                            if span.get('html', ''):
                                para_content[_BlockType.TABLE_BODY] = _format_embedded_html(span['html'], img_buket_path)
            if block['type'] == _BlockType.TABLE_CAPTION:
                para_content[_BlockType.TABLE_CAPTION].append(merge_para_with_text(block))
    elif para_type == _BlockType.CHART:
        para_content = {
            'type': _ContentType.CHART,
            'img_path': '',
            'content': '',
            _BlockType.CHART_CAPTION: [],
        }
        for block in para_block['blocks']:
            if block['type'] == _BlockType.CHART_BODY:
                for line in block['lines']:
                    for span in line['spans']:
                        if span['type'] == _ContentType.CHART:
            ...
            if block['type'] == _BlockType.CHART_CAPTION:
                para_content[_BlockType.CHART_CAPTION].append(merge_para_with_text(block))
...
def make_blocks_to_content_list_v2(para_block, img_buket_path):
    ...
    if para_type in [
        ...
        if para_type == _BlockType.HEADER:
            content_type = _ContentTypeV2.PAGE_HEADER
        elif para_type == _BlockType.FOOTER:
            content_type = _ContentTypeV2.PAGE_FOOTER
        else:
        ...
        para_content = {
            'type': _ContentTypeV2.EQUATION_INTERLINE,
            'content': {
        ...
        if (
            ...
            table_type = _ContentTypeV2.TABLE_COMPLEX
        else:
            table_type = _ContentTypeV2.TABLE_SIMPLE

        for block in para_block['blocks']:
            if block['type'] == _BlockType.TABLE_CAPTION:
                table_caption.extend(merge_para_with_text_v2(block))
        para_content = {
            'type': _ContentTypeV2.TABLE,
            'content': {
        ...
        list_type = _ContentTypeV2.LIST_TEXT
        attribute = para_block.get('attribute', 'unordered')
        para_content = {
            'type': _ContentTypeV2.LIST,
            'content': {
                'list_type': list_type,
                'attribute': attribute,
...
def get_body_data(para_block):
    ...
    def get_data_from_spans(lines):
        for line in lines:
            for span in line.get('spans', []):
                span_type = span.get('type')
                if span_type == _ContentType.TABLE:
                    return span.get('image_path', ''), span.get('html', '')
                elif span_type == _ContentType.CHART:
                    return span.get('image_path', ''), span.get('content', '')
                elif span_type == _ContentType.IMAGE:
                    return span.get('image_path', ''), ''
                elif span_type == _ContentType.INTERLINE_EQUATION:
                    return span.get('image_path', ''), span.get('content', '')
                elif span_type == _ContentType.TEXT:
    ...
    if 'blocks' in para_block:
        for block in para_block['blocks']:
            ...
            if block_type in [_BlockType.IMAGE_BODY, _BlockType.TABLE_BODY, _BlockType.CHART_BODY, _BlockType.CODE_BODY]:
                result = get_data_from_spans(block.get('lines', []))
                if result != ('', ''):
                    return result
                if block_type == _BlockType.CHART_BODY:
                    return result
        return '', ''
    ...
    for i, line in enumerate(para_block['lines']):
        for j, span in enumerate(line['spans']):
            ...
            if content.strip() or (content and has_visible_style):
                if span['type'] == _ContentType.INLINE_EQUATION:
                    span['type'] = _ContentTypeV2.SPAN_EQUATION_INLINE
                para_content.append(span)
    return para_content


def _union_make(pdf_info_dict: list,
               make_mode: str,
               img_buket_path: str = '',
               ):
    ...
    for page_info in pdf_info_dict:
        ...
        if make_mode in [_MakeMode.MM_MD, _MakeMode.NLP_MD]:
            if not paras_of_layout:
                continue
            page_markdown = _mk_blocks_to_markdown(paras_of_layout, make_mode, img_buket_path,
                                                   page_idx=page_idx)
            output_content.extend(page_markdown)
        elif make_mode == _MakeMode.CONTENT_LIST:
        ...
        elif make_mode == _MakeMode.CONTENT_LIST_V2:
    ...
    if make_mode in [_MakeMode.MM_MD, _MakeMode.NLP_MD]:
        return '\n\n'.join(output_content)
    elif make_mode in [_MakeMode.CONTENT_LIST, _MakeMode.CONTENT_LIST_V2]:

...
Version: 1.0
  Date: 2026-04-10
  Description: DOCX 解析核心类（来自 mineru/model/docx/docx_converter.py）
'''
...
from typing import Any, BinaryIO, Callable, Final, Iterator, Literal, Optional, Union
...
from .constants import _BlockType, _Formatting, _Script
from .latex_dict import BLANK
from .omml import _oMath2Latex


def _str_sha256(input_string: str) -> str:
...
def _image_to_b64str(image: Image.Image, image_format: str = 'JPEG') -> str:
    buf = BytesIO()
...
def _tie_up_category_by_index(
    ...
    def calc_effective_index_diff(obj_index: int, sub_index: int) -> int:
        ...
        abs_diff = end - start
        other_objects_count = sum(
            1 for idx in range(start + 1, end) if idx in object_indices
        )
    ...
    for obj in objects:
        ...
        for i, subject in enumerate(subjects):
            index_diff = calc_effective_index_diff(obj_index, subject['index'])
...
def _parse_xml_str(xml_str, namespace_mapping=None):
...
def _read_str(xml_str):
    i = _parse_xml_str(xml_str, _namespaces)
    return _collapse_alternate_content(i)[0]


# ── DOCX Converter (from mineru/model/docx/docx_converter.py) ───────────────
...
class _DocxConverter:
    ...
    def __init__(self):
        ...
        self.blip_xpath_expr = etree.XPath(
            ".//a:blip", namespaces=_DocxConverter._BLIP_NAMESPACES
        )
    ...
    def _build_text_from_elements(
        ...
        paragraph_elements: list[
            tuple[str, Optional[_Formatting], Optional[Union[AnyUrl, Path, str]]]
        ],
    ) -> str:
        """
    ...
    def _collect_toc_anchor_set(self) -> set[str]:
        ...
        for hl in self.docx_obj.element.body.findall(
            ".//w:hyperlink", namespaces=_DocxConverter._BLIP_NAMESPACES
        ):
    ...
    def _walk_linear(
        ...
        for element in body:
            ...
            drawingml_els = element.findall(
                ".//w:drawing", namespaces=_DocxConverter._BLIP_NAMESPACES
            )
            if drawingml_els:
            ...
            if element not in self.processed_textbox_elements:
                ...
                txbx_xpath = etree.XPath(
                    ...
                    namespaces=_DocxConverter._BLIP_NAMESPACES,
                )
            ...
            # 检查 sdt 元素
                ...
                sdt_content = element.find(
                    ".//w:sdtContent", namespaces=_DocxConverter._BLIP_NAMESPACES
                )
                if sdt_content is not None:
                    if self._is_toc_sdt(element):
                        ...
                        paragraphs = sdt_content.findall(
                            ".//w:p", namespaces=_DocxConverter._BLIP_NAMESPACES
                        )
                        for p in paragraphs:
    ...
    def _build_paragraph_html_with_equations(self, xml_para) -> Optional[str]:
        ...
        for subt in xml_para.iter():
            ...
            # OMML 公式元素（排除 oMathPara 容器避免重复处理）
                ...
                try:
                    latex = str(_oMath2Latex(subt)).strip()
                    if latex:
    ...
    def _handle_text_elements(
        ...
        if element.find(".//w:sectPr", namespaces=_DocxConverter._BLIP_NAMESPACES) is not None:
            # 如果没有text内容
            if element.text == "":
                self.cur_page = []
                self.pages.append(self.cur_page)
        ...
        for image in drawing_blip:
            ...
            if image_data is None:
                ...
                if pil_image.format in ("WMF", "EMF"):
                    if _is_windows_environment():
                        # 在 Windows 上，Pillow 依赖底层的 Image.core.drawwmf 渲染
                        # 有时需要显式调用 .load() 确保矢量图被光栅化到内存中
                        try:
                            pil_image.load()
                            img_base64 = _image_to_b64str(pil_image, image_format="PNG")
                        except OSError as e:
                            ...
                            img_base64 = _image_to_b64str(placeholder, image_format="JPEG")
                    else:
                    ...
                    if pil_image.mode != "RGB":
                        ...
                        img_base64 = _image_to_b64str(pil_image, image_format="JPEG")
                image_block = {
    ...
    def _get_paragraph_elements(self, paragraph: Paragraph):
        ...
        Returns:
            list[tuple[str, Optional[_Formatting], Optional[Union[AnyUrl, Path, str]]]]:
    ...
    @classmethod
    def _get_format_from_run(cls, run: Run) -> Optional[_Formatting]:
        """
        从 Run 对象获取格式信息。
        ...
        script = _Script.SUB if is_sub else _Script.SUPER if is_sup else _Script.BASELINE
    ...
    def _handle_equations_in_text(self, element, text):
        ...
        for subt in element.iter():
            ...
            if tag_name == "t" and "math" not in subt.tag:
                ...
                latex_equation = str(_oMath2Latex(subt)).strip()
    ...
    def _is_toc_sdt(self, element: BaseOxmlElement) -> bool:
        ...
        sdt_pr = element.find("w:sdtPr", namespaces=_DocxConverter._BLIP_NAMESPACES)
        if sdt_pr is not None:
            doc_part_gallery = sdt_pr.find(
                ".//w:docPartGallery", namespaces=_DocxConverter._BLIP_NAMESPACES
            )
            ...
            # 检查 tag 元素的值
            tag_elem = sdt_pr.find("w:tag", namespaces=_DocxConverter._BLIP_NAMESPACES)
    ...
    def _extract_paragraph_bookmark(self, paragraph_element: BaseOxmlElement) -> Optional[str]:
        ...
        for bm in paragraph_element.findall(
            ".//w:bookmarkStart", namespaces=_DocxConverter._BLIP_NAMESPACES
        ):
    ...
    def _is_caption(self, element: BaseOxmlElement) -> bool:
        ...
        instr_texts = element.findall(
            ".//w:instrText", namespaces=_DocxConverter._BLIP_NAMESPACES
        )
    ...
    def _handle_drawingml(self, elements: list[BaseOxmlElement]):
        ...
        for element in elements:
            chart = element.find(
                ".//c:chart", namespaces=_DocxConverter._BLIP_NAMESPACES
            )

...
Version: 1.0
  Date: 2026-04-10
  Description: OMML（Office Math Markup Language）转 LaTeX 转换器
               来源：mineru/model/docx/tools/math/omml.py
'''
...
from .latex_dict import (
    CHARS, BLANK, BACKSLASH, ALN,
    CHR, CHR_BO, T, FUNC, FUNC_PLACE, BRK, CHR_DEFAULT,
    POS, POS_DEFAULT, SUB, SUP, F, F_DEFAULT,
    D, D_DEFAULT, RAD, RAD_DEFAULT, ARR,
...
class Tag2Method:
    ...
    def process_children_dict(self, elm, include=None):
        ...
        """
        res = {}
        for stag, t, _e in self.process_children_list(elm, include=include):
            res[stag] = t
        return res
    ...
    def process_children(self, elm, include=None):
        ...
        """
        return BLANK.join([t for stag, t, _e in self.process_children_list(elm, include=include)])

...
Version: 1.0
  Date: 2026-04-10
  Description: Magic Model 与块分类/关联工具
'''

import re
from typing import Literal, Callable

from .constants import _BlockType, _ContentType
    ...
    while pos < len(content):
        ...
        if next_tag_type == 'eq':
            ...
            if eq_end != -1:
                ...
                spans.append({
                    "type": _ContentType.INLINE_EQUATION,
...
def ___tie_up_category_by_index(blocks, subject_block_type, object_block_type):
    """基于index的主客体关联包装函数"""
    # 定义获取主体和客体对象的函数
    def get_subjects():
        return list(
    ...
    # 调用通用方法
    return _tie_up_category_by_index(
        get_subjects,
        get_objects,
        include_bbox=False,
    )
...
def get_type_blocks(blocks, block_type: Literal["image", "table", "chart"]):
    with_captions = ___tie_up_category_by_index(blocks, f"{block_type}_body", f"{block_type}_caption")
    ret = []
...
class _MagicModel:
    def __init__(self, page_blocks: list):
        ...
        for index, block_info in enumerate(page_blocks):
            ...
            if not block_content and block_type != _BlockType.CHART:
                continue
            ...
            if block_type in [
                ...
                block_type = _BlockType.INTERLINE_EQUATION
                span = {
                    "type": _ContentType.INTERLINE_EQUATION,
                    "content": block_content,
                }
            ...
            if (
                ...
                and anchor.strip()
                and block_type in [_BlockType.TITLE, _BlockType.TEXT, _BlockType.INTERLINE_EQUATION]
            ):
                block["anchor"] = anchor.strip()
            if block_type == _BlockType.TITLE:
        ...
        self.index_blocks = []
        for block in blocks:
            if block["type"] in [_BlockType.IMAGE_BODY, _BlockType.IMAGE_CAPTION, _BlockType.IMAGE_FOOTNOTE]:
                self.image_blocks.append(block)
            elif block["type"] in [_BlockType.TABLE_BODY, _BlockType.TABLE_CAPTION, _BlockType.TABLE_FOOTNOTE]:
                self.table_blocks.append(block)
            elif block["type"] in [_BlockType.CHART_BODY, _BlockType.CHART_CAPTION]:
                self.chart_blocks.append(block)
            elif block["type"] == _BlockType.INTERLINE_EQUATION:
                self.interline_equation_blocks.append(block)
            elif block["type"] == _BlockType.TEXT:
                self.text_blocks.append(block)
            elif block["type"] == _BlockType.TITLE:
                self.title_blocks.append(block)
            elif block["type"] in [_BlockType.REF_TEXT]:
                self.ref_text_blocks.append(block)
            elif block["type"] in [_BlockType.PHONETIC]:
                self.phonetic_blocks.append(block)
            elif block["type"] in [_BlockType.HEADER, _BlockType.FOOTER, _BlockType.PAGE_NUMBER, _BlockType.ASIDE_TEXT, _BlockType.PAGE_FOOTNOTE]:
                self.discarded_blocks.append(block)
            elif block["type"] == _BlockType.LIST:
                self.list_blocks.append(block)
            elif block["type"] == _BlockType.INDEX:
                self.index_blocks.append(block)
            else:
                continue

        self.image_blocks, not_include_image_blocks = fix_two_layer_blocks(self.image_blocks, _BlockType.IMAGE)
        self.table_blocks, not_include_table_blocks = fix_two_layer_blocks(self.table_blocks, _BlockType.TABLE)
        self.chart_blocks, not_include_chart_blocks = fix_two_layer_blocks(self.chart_blocks, _BlockType.CHART)

        for block in not_include_image_blocks + not_include_table_blocks + not_include_chart_blocks:
            block["type"] = _BlockType.TEXT
            self.text_blocks.append(block)
